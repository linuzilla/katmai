...
DH *tmp_dh_callback (SSL *ssl, int is_export, int keylength) {
	...
	if (! dh512 || ! dh1024) init_dhparams ();

	switch (keylength) {
	case 512:
		ret = dh512;
		break;
	case 1024: // generating DH params is too costly to do on the fly
	default:
		ret = dh1024;
		break;
	}
...
int main (int argc, char *argv[]) {
	...
	fprintf (stderr, "init OpenSSL ... ");
	init_OpenSSL ();
	fprintf (stderr, "ok.\n");

	fprintf (stderr, "seed_prng ... ");
	seed_prng (8192);
	fprintf (stderr, "ok.\n");

	fprintf (stderr, "setup server ctx ...");
