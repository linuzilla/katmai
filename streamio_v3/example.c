...
void client (void) {
	...
	char	*f1   = "/home/cc/liujc/FoveonX3/run/99812.org";
	char	*file = "/home/cc/liujc/FoveonX3/run/805200";
	char	*fout = "/tmp/file.save";
	/*
	...
	*/
	...
	if (cli->openfile (cli, 1, f1) >= 0) {
		for (i = 0; i < 20; i++) {
			len = cli->read (cli, 1, buffer, sizeof buffer);
	...
	if (cli->openfile (cli, 0, file) >= 0) {
		...
		cli->openfile (cli, 1, f1);

		while ((len = cli->read (cli, 0, buffer, sizeof buffer)) > 0) {
			write (fd, buffer, len);
			cli->read (cli, 1, buffer, sizeof buffer / 2);
		}
